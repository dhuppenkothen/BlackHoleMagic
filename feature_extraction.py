...
def extract_data(filename, val=True, train_frac=0.5, validation_frac=0.25, test_frac = 0.25,
...
def make_features(seg, bins=30, navg=4, hr_summary=True, ps_summary=True, lc=True, hr=True):
    features = []
    if lc:
        lc_all = []
    if hr:
    ...
    for s in seg:
        ...
        if hr_summary:
            ...
            xedges, yedges, h = hr_maps(s, bins=bins)
